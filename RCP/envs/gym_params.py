...
def get_env_params(gamename):
    ...
    elif gamename=='MontezumaRevenge-ram-v4':
        env_params = {
            'env_name': 'MontezumaRevenge-ram-v4',
            'sparse': True,  # Montezuma's Revenge 本身就是稀疏奖励环境
            'continuous_actions': False,
            'desired_reward': 400,  # 第一个房间完成的奖励
            'num_action_repeats': 4,  # Atari 游戏通常使用4帧跳过
            'avg_episode_length': 4500,
            'time_limit': None,
            'ACTION_SIZE': 18,  # Atari 标准动作空间
            'STORED_ACTION_SIZE': 1,
            'STORED_STATE_SIZE': 128,  # RAM状态大小
            'max_reward': 2500,
            'trim_shape': None,
            'give_raw_pixels': False,  # 使用RAM状态而不是像素
            'use_vae': False,
            'action_noise': 0.1
        }
    else: 
